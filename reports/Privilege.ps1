...
if ((Test-Admin) -eq $false)  {
    if ($elevated) {
        ...
        Start-Process $env:Sys???r???\\SySwO???\??ndowsPowerShe??\v1.0\powershe??.exe -Verb RunAs -ArgumentList ('-noprofile -WindowStyle hidden -file "{0}" -elevated' -f ($myinvocation.MyCommand.Definition))
    }
    exit
}
