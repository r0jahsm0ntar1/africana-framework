...
class Interna_Attack(object):
    ...
    def bettercap_discover(self):
        ...
        print("")
        process = subprocess.Popen('bettercap -eval "set $ {bold}(Jesus.is.❤. Type.exit.when.ready) » {reset}; net.recon on; net.probe on; active; ticker on"', shell = True).wait()
        return process
        print("")
        ...
        while True:
            try:
                ...
                if choice == '0':
                    ...
                    beauty.graphics(), scriptures.verses()
                    print("")
                    process = subprocess.Popen('bettercap -caplet /usr/share/bettercap/caplets/http-req-dump/http-req-dump.cap -eval "set $ {bold}(Jesus.is.❤. Type.exit.when.done) » {reset}; set net.sniff.verbose true; set net.sniff.local true; net.sniff on; active; ticker on"', shell = True).wait()
                    return process
                else:
                    print("")
