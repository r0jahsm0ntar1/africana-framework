...
func confiGure() {
    if strings.Contains(flaG(), "anonsurf") {
        fmt.Printf("\n%sanonsurf is already running....%s\n", bcolors.BLUE, bcolors.ENDC)
        os.Exit(0)
    }
    ...
    chameLeons()
    exec.Command("service", "tor", "restart").Run()
...
func termiNate() {
    ...
    if _, err := os.Stat("/etc/resolv.conf.backup_anonsurf"); err == nil {
        ...
        cmd.Run()
        fmt.Printf("\n%s%sReverting to default resolv.conf%s\n", bcolors.GREEN, bcolors.BOLD, bcolors.ENDC)
        time.Sleep(500 * time.Millisecond)
    ...
    if _, err := os.Stat("/etc/tor/torrc.bak_anonsurf"); err == nil {
        ...
        cmd.Stdin = strings.NewReader("yes\n")
        cmd.Run()
        fmt.Printf("%s%sDropping of torrc file%s\n", bcolors.GREEN, bcolors.BOLD, bcolors.ENDC)
        time.Sleep(500 * time.Millisecond)
    ...
    if strings.Contains(flaG(), "anonsurf") {
        trigger++
        fmt.Println(bcolors.GREEN + bcolors.BOLD + "Restoring Iptables rules.." + bcolors.ENDC)
    ...
    if trigger == 0 {
        ...
        fmt.Printf("%s%sCleaning up complete :)%s\n", bcolors.DARKGREEN, bcolors.BOLD, bcolors.ENDC)
        fmt.Println()
    }
}
...
func torCircuit() {
    ...
    time.Sleep(400 * time.Millisecond)
    fmt.Printf("%s%sDONE :)%s\n", bcolors.Colors(), bcolors.BOLD, bcolors.ENDC)
    time.Sleep(400 * time.Millisecond)
    fmt.Printf("%sYour new IP appears to be: %s%s%s\n", bcolors.GREEN, bcolors.ENDC, bcolors.Colors(), bcolors.BOLD, iP())
}

func torStart(Timer float64) {
    resp, err := http.Get("https://check.torproject.org")
    if err != nil {
        Timer++
        if Timer > 2 {
            fmt.Printf("%s\nUnable to get the network details, try with option 6. If this persists, please raise an issue on GitHub.\n\n%s%s%s\n", bcolors.Colors(), bcolors.RED, err, bcolors.ENDC)
            fmt.Println()
            return
        }
        fmt.Printf("%s\nHaving trouble fetching exit-node details, %sretrying....%s\n", bcolors.GREEN, bcolors.CYAN, bcolors.ORANGE)
...
func configureFirewall() {
    fmt.Printf("%sBacking up Iptables..%s\n", bcolors.GREEN, bcolors.ENDC)
    ...
    if strings.TrimSpace(firewallGreen) == "0" {
        ...
        cmd.Run()
        fmt.Printf("%sDone :) %slocation: %s/etc/iptables_rules_anonsurf.bak%s\n", bcolors.RED, bcolors.BLUE, bcolors.RED, bcolors.ENDC)
    }
...
`
    fmt.Printf("%sCooking up firewall rules%s\n", bcolors.GREEN, bcolors.ENDC)
...
func resetToDefault(overidePass bool, resetAsChildFunc bool) {
    ...
    if !overidePass {
        ...
        defaultCheckGreen, defaultCheckRed := checkDefaultRules()
        if defaultCheckRed != "" {
            fmt.Printf("%sError while checking existing rules; %sexiting..\n%sError message: %s%s%s\n", bcolors.RED, bcolors.ORANGE, bcolors.YELLOW, bcolors.Colors(), defaultCheckRed, bcolors.ENDC)
            os.Exit(1)
        }
        if strings.TrimSpace(defaultCheckGreen) != "0" {
...
func AnonymityStart() {
    utils.ClearScreen(); banners.Banner(); scriptures.Verse(); confiGure(); configureFirewall(); torStart(0)
}

func AnonymityStop() {
    utils.ClearScreen(); banners.Banner(); scriptures.Verse(); termiNate()
}

func AnonymityStatus() {
    utils.ClearScreen(); banners.Banner(); scriptures.Verse()
    fmt.Printf("\n%sYour IP is: %s%s%s%s\n", bcolors.GREEN, bcolors.ENDC, iP(), bcolors.Colors(), bcolors.ENDC)
    fmt.Println()
}

func AnonymityChains() {
