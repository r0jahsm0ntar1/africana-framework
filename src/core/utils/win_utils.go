...
func init() {
	...
	PartialSuggestions = map[string]map[string]string{
		"set": {
			"lhost": " <ip_address>",
			"lport": " <port_number>",
			"rhost": " <ip_address>",
	...
	ValueSuggestions = map[string]string{
	...
	CommandCategories = map[string][]string{
		...
		"Information":   {"show", "help", "version"},
		"Execution":     {"run", "use", "scan"},
	}

	CommonValues = []string{"true", "false", "enable", "disable"}

	AvailableCommands = []string{
		"set", "show", "run", "use", "scan", "help", "exit",
		"start", "stop", "config", "version", "status",
	}
	...
	CommandPredictions = map[string][]string{
		"set":   {"lhost", "lport", "rhost", "target", "protocol"},
		"show":  {"options", "config", "status", "version"},
		"run":   {"scan", "attack", "module"},
		"use":   {"scanner/network", "exploit/windows"},
	}
...
func (le *LineEditor) handleTabCompletion() {
	...
	if isDoubleTab {
		...
		// If no predictions, insert a tab character
		le.insertChar('\t')
	}
...
func (le *LineEditor) ReadLine() (string, error) {
	...
	return le.windowsReadLine()
}

func (le *LineEditor) windowsReadLine() (string, error) {
	fmt.Print(le.prompt)
	...
	for {
		...
		case 0x1B: // Escape sequence (arrow keys)
			// Try to read the next character to detect arrow keys
