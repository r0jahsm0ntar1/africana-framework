...
func AskForProxy() *url.URL {
    ...
    for {
        fmt.Printf(bcolors.UNDERL + bcolors.BOLD + "afr3" + bcolors.ENDC + "websites(" + bcolors.RED + "src/webattackers/askfor_proxy(eg.http://localhost:80)_func.go" + bcolors.ENDC + ")" + bcolors.GREEN + " > " + bcolors.ENDC)
...
func Editors(filesToReplacements map[string]map[string]string) {
    for fileName, replacements := range filesToReplacements {
        ...
        if err != nil {
            fmt.Printf(bcolors.RED + "[!] " + bcolors.DARKCYAN + "Configuring file: " + bcolors.ENDC + "%s %v" + bcolors.ENDC, fileName, err)
        } else {
            fmt.Printf(bcolors.GREEN + "[*] " + bcolors.ENDC + "Succesfully configured file: %s%s!\n" + bcolors.ENDC, bcolors.RED, fileName)
        }
    }
}
