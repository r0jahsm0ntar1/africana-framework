...
def tor_stat(e_langa):
    try:
        ...
        e_langa += 1
        if e_langa > 2:
            print(bcolors.BLUE + "[" + bcolors.RED + "         Unable to get the network-details, try Option 5. " + bcolors.BLUE + "]" + bcolors.ENDC)
            sys.exit(1)
        print(bcolors.ENDC + "\n~>[" + bcolors.GREEN + " having trouble fetching exit-node details,"  + bcolors.CYAN + "retrying..."  + "{e_langa}" + bcolors.ENDC + " ]<~" + bcolors.ENDC)
        time.sleep(1.2)
