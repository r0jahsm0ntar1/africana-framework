...
<#
.SYNOPSIS
Converts pwsh scripts to standalone executables.
.DESCRIPTION
Converts pwsh scripts to standalone executables. GUI output and input is activated with one switch,
...
.LINK
https://www.pwshgallery.com/packages/ps2exe
.LINK
...
<################################################################################>
	...
	{
		Write-Output "pwsh Desktop environment started...`n"
	}
	...
	{ # starting Windows Powershell
		...
		pwsh -Command "&'$($MyInvocation.MyCommand.Name)' $CallParam"
		return
	}
	...
	if (!$MTA -and !$STA)
	{
		# Set default apartment mode for pwsh version if not set by parameter
		$STA = $TRUE
	}
	...
	if (!$noConsole)
	{
		if ([System.AppDomain]::CurrentDomain.GetAssemblies() | Where-Object { $_.ManifestModule.Name -ieq "Microsoft.pwsh.ConsoleHost.dll" })
		{
			$referenceAssembies += ([System.AppDomain]::CurrentDomain.GetAssemblies() | Where-Object { $_.ManifestModule.Name -ieq "Microsoft.pwsh.ConsoleHost.dll" } | Select-Object -First 1).Location
		}
	}
...
"@
	...
	$programFrame = @"
// Simple pwsh host created by Ingo Karstein (http://blog.karstein-consulting.com)
// Reworked and GUI support by Markus Scholtes
...
"@ })
						...
						for (int i = separator; i < args.Length; i++)
						{
							...
							{ // parameter in pwsh style, means named parameter found
								...
								{ // store named parameter in buffer
									...
									// caution: when called in pwsh $TRUE gets converted, when called in cmd.exe not
									...
									{ // switch found
										...
										// caution: when called in pwsh $FALSE gets converted, when called in cmd.exe not
...
"@
	...
	if ($cr.Errors.Count -gt 0)
	{
		...
		Write-Error -ErrorAction Continue "Could not create the pwsh .exe file because of compilation errors. Use -verbose parameter to see details."
